...
| **6** | 20-line HTML page allocating a 1024×512×41 R8 `TEXTURE_2D_ARRAY` on the **actual target laptop**. Log `MAX_TEXTURE_SIZE`, `MAX_ARRAY_TEXTURE_LAYERS`. | $0 | 20 min | Whether the client rendering premise holds on integrated graphics. Design to the spec floors, not to the 92%-report statistic. |
