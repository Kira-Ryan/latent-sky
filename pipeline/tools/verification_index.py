...
{table}
  ...
  <footer>
    ...
    <span>{count} run{plural} scored</span>
  </footer>
</div>
</body>
</html>
...
def main(argv: list[str] | None = None) -> None:
    ...
    ap.add_argument("--extra", type=pathlib.Path, default=None,
                    help="JSON array of extra rows for runs this checkout does not hold "
                         "(the daily pipeline publishes straight to the bucket)")
    args = ap.parse_args(argv)

    rows = rows_from(args.results, args.pages)
    if args.extra and args.extra.is_file():
        rows += json.loads(args.extra.read_text(encoding="utf-8"))
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(
        PAGE.format(table=render(rows), count=len(rows), plural="" if len(rows) == 1 else "s"),
        encoding="utf-8",
    )
    print(f"wrote {args.out} ({args.out.stat().st_size:,} bytes, {len(rows)} run(s))")
    for r in sorted(rows, key=lambda r: r["init"], reverse=True):
        print(f"  {r['init'][:16]:18} {r['id']:22} -> {r['page']}")
