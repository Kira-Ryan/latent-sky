...
<div class="wrap">
  ...
  observations beside it. The results are what they are; the rules that produce them are
  stated, versioned and dated in <a href="/verification/protocol.html">the verification protocol</a>.</p>
...
const summarise = (h) => {
  ...
  if (h.usefulScaleKm === null || h.usefulScaleKm === undefined) {
    return `No hour above the useful line at ${h.thresholdDbz} dBZ to ${largest} km`;
  }
  return `${h.usefulHours} of ${h.scoredHours} h above the line at ${Math.round(h.usefulScaleKm)} km &middot; first rule`;
};
