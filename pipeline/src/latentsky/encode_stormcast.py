...
def frame_times(coarse, hero) -> list[str]:
    ...
    The two stores must describe the same forecast. Coarse frames are read by
    index alongside hero frames, so a hero store from another day, or a coarse
    lead axis that differs from the hero's, would produce a schema-valid
    manifest whose frames are labelled with one run's times and filled with
    another's fields. The daily pipeline writes both stores in one process, so
    this guards the hand-run and mis-untarred cases; it costs nothing.
    """
...
def encode_layers(
    ...
    plan = [
        ...
        ("refc-fine", "hero-fine", "refc", hero_field("refc"), fine_index,
         " — generated, 3 km (no coarse layer: a 25 km model cannot place individual storms)", None, 3.0),
    ...
    if fss_path is not None:
        ...
        # The results must be THIS run's. A results file from another run would
        # otherwise put its figure on this run's globe as a scored verification,
        # which is the one false claim this site must never make. The scorer
        # records the initialisation, the frame count and the member count of
        # what it scored; all three must match what is being encoded.
        expected = {"init": init_iso, "leads": nframes, "members": len(member_paths) or None}
        got = {"init": results.get("init"), "leads": len(results.get("leads") or []),
               "members": results.get("members")}
        if got != expected:
            raise EncodeStormcastError(
                f"{fss_path} scores a different run: it describes {got}, this run is {expected}"
            )
...
GENERATED_NOTE = (
    ...
    "forecast — one plausible realisation, not observed truth. No reflectivity is "
    "shown on the coarse side: a 25 km global model such as GFS does emit a diagnosed "
    "composite reflectivity, but with convection parameterised it cannot place "
    "individual storms, which is what the 3 km layer exists to show."
)
...
ENSEMBLE_NOTE = (
    ...
    "composite reflectivity at or above 40 dBZ in each cell. Agreement measures how "
    "much the sampler alone moves the answer with the initial condition held fixed; "
    "it is not a calibrated probability, and it cannot see an error the members share."
)
