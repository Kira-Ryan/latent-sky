...
Produces, per Architecture.md sections 5.5, 7.1 and 7.2:
  luts/<var>.lut.png       256x1 RGBA LUTs (batlowK for wind10m, thermal for t2m)
  frames/*.webp            2 frames x {fine 64x64, coarse 8x8} x 2 variables,
                           WebP lossless with exact alpha preservation
...
FRAME_TIMES = ["2026-01-01T00:00:00Z", "2026-01-01T06:00:00Z"]
...
def synthetic_field(var: str, frame: int, n: int) -> np.ndarray:
    ...
    if var == "wind10m":
        ...
        cx, cy = (0.35, 0.40) if frame == 0 else (0.62, 0.58)
        r2 = (xx - cx) ** 2 + (yy - cy) ** 2
